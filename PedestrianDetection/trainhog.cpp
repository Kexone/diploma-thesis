#include "trainhog.h"
#include <QDebug>
void TrainHog::fillVectors(std::vector<std::string> &samplesListPath, bool isNeg)
{
    assert(!samplesListPath.empty());
    ...
    for(auto &sample : samplesListPath) {
        frame = cv::imread(sample,1);
